...
class TorchMD_ET(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.x_embedding = nn.Embedding(self.max_atom_type, hidden_channels)
        ...
        # self.cond_embed = nn.Linear(2, hidden_channels)
        ...
        for _ in range(num_layers):
            ...
            self.temb_layers.append(t_layer)
        ...
        # self.vel_embed = nn.Linear(1, hidden_channels // 2)
        self.acc_embed = nn.Linear(1, hidden_channels)

        self.vec_norm = nn.LayerNorm(hidden_channels)
        self.out_norm = nn.LayerNorm(hidden_channels)

        self.vec_head = nn.ModuleList([
            GatedEquivariantBlock(
                hidden_channels,
                hidden_channels // 2,
    ...
    def reset_parameters(self):
        ...
        self.acc_embed.reset_parameters()
        # self.vel_embed.reset_parameters()
    ...
    def forward(self,
                ...
                acc: Tensor,
                batch: Tensor,
                edge_index: Tensor=None,  # Pre-computed edge index
                edge_vec: Tensor=None,  # Pre-computed edge vectors
                edge_weight: Tensor=None,  # Pre-computed edge weights
                t: Tensor=None,  # Pre-computed time
                ) -> Tuple[Tensor, Tensor]:

        x = self.x_embedding(feat)

        temb = self.t_embedding(t)

        edge_index, edge_weight, edge_vec = self.distance(
            pos, batch, edge_index=edge_index, edge_weight=edge_weight, edge_vec=edge_vec
        )
        assert (
        ...
        if self.neighbor_embedding is not None:
            x = self.neighbor_embedding(feat, x, edge_index, edge_weight, edge_attr)

        # add velocity and predicted acceleration as conditional information
        vec = torch.unsqueeze(acc, -1)
        vec = self.acc_embed(vec)

        vec = torch.zeros(x.size(0), 3, x.size(1), device=x.device)

        for i, (attn, t_layer) in enumerate(zip(self.attention_layers, self.temb_layers)):
    ...
    def load_my_state_dict(self, state_dict):
        ...
        for name, param in state_dict.items():
            curr_name = name.replace('module.', '')
            if curr_name not in own_state:
                print(name, 'NOT loaded!')
                continue
            if isinstance(param, nn.parameter.Parameter):
                # backwards compatibility for serialized parameters
                param = param.data
            own_state[curr_name].copy_(param)
            print(name, 'loaded!')
