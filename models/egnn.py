...
class E_GCL_vel(E_GCL):
    ...
    def forward(self, data, h, coord, vel, disp, edge_index, edge_attr=None, node_attr=None):
        __, coord_diff, __ = get_pbc_distances(
            coord,
            edge_index,
            data.cell,
        ...
        coord += self.coord_mlp_vel(h) * vel + self.coord_mlp_disp(h) * disp
        h, agg = self.node_model(h, edge_index, edge_feat, node_attr)
        ...
        for i in range(0, self.n_layers):
            if i == 0:
                h, x, _ = self._modules["gcl_%d" % i](
                    data, h, data.pos, data.vel, data.disp, data.edge_index, data.edge_attr
                )
            else:
                h, x, _ = self._modules["gcl_%d" % i](
                    data, h, x, data.vel, data.disp, data.edge_index, data.edge_attr
                )
            h += temb
