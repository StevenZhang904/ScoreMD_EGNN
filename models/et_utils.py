...
class NeighborEmbedding(MessagePassing):
    def __init__(self, hidden_channels, num_rbf, cutoff_lower, cutoff_upper, max_z=100):
        ...
        # self.embedding = nn.Linear(6, hidden_channels)
...
class Distance(nn.Module):
    ...
    def forward(self, pos, batch, edge_index=None, edge_weight=None, edge_vec=None):
        if edge_index is None:
            warnings.warn('No edge_index provided, fall back to pygeom radius graph, no perioidic boundary conditions')
            ...
            if self.loop:
                ...
                # NOTE: might influence force predictions as self loop gradients are ignored
                mask = edge_index[0] != edge_index[1]
                mask_edge_weight = torch.zeros(edge_vec.size(0), device=edge_vec.device)
                mask_edge_weight[mask] = torch.norm(edge_vec[mask], dim=-1)
                edge_weight = mask_edge_weight
